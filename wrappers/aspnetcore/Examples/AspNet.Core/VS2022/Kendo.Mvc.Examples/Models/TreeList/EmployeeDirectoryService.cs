namespace Kendo.Mvc.Examples.Models.TreeList
{
    ...
    {        
        public static EmployeeDirectoryModel ToEmployeeDirectoryModel(this EmployeeDirectory employee, bool hasChildren)
        {
            ...
            using (var db = GetContext())
            {
                ...
                if (result == null || UpdateDatabase)
                {
                    result = db.EmployeeDirectories
                        .ToList()
                        .Select(employee => employee.ToEmployeeDirectoryModel(db.EmployeeDirectories.Where(s => s.ReportsTo == employee.EmployeeID).Count() > 0))
                        .ToList();
        ...
        public virtual IList<EmployeeDirectoryRemoteModel> GetAllRemote()
        {
            using (var db = GetContext())
            {
                return db.EmployeeDirectories
                        .ToList()
                        .Select(employee => employee.ToEmployeeDirectoryRemoteModel(db.EmployeeDirectories.Where(s => s.ReportsTo == employee.EmployeeID).Count() > 0))
                        .ToList();
            }
        }
